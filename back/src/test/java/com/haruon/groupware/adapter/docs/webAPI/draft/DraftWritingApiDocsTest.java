...
import com.haruon.groupware.adapter.webapi.draft.DraftWritingApi;
...
public class DraftWritingApiDocsTest extends RestDocsSupport {
    ...
    void update_general_draft() throws Exception {
        mockMvc.perform(
                        patch(REQUEST_MAPPING_URL + "/general/{draftId}", 10L)
                                .with(employeeAuthentication())
                                .header("Authorization", "Bearer accessToken")
                                .contentType(MediaType.APPLICATION_JSON)
