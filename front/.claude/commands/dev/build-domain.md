...
## 5-1. 더미데이터 주입(필요 시) — `@dummy-data-seeder`

- 5단계에서 등록 API가 붙었는데 목록이 비어 있어 화면/기능 확인이 어려운 경우에만,
`@dummy-data-seeder`에게 `$1` 도메인과 관련된 등록 기능ID를 명시해 더미데이터 작성을 위임한다.
- 필수 단계가 아니다 — 이미 데이터가 있거나 확인에 지장이 없으면 건너뛴다.
- 이 에이전트는 REST 등록 API만 호출하며 DB에 직접 접근하지 않는다. 생성된 더미데이터는
삭제·롤백하지 않는다(등록 전용). 임시 시드 스크립트는 scratchpad에서만 실행되고 저장소에는
남지 않는다.
...
- 향후 별도의 UI/UX 작업을 최소한의 기능으로만 뷰를 배치한다.
