...
## 3. UX 검토 (`$3` = true 시 에만 )
- 전제1: dev 서버(localhost:5173, 필요 시 백엔드 localhost:8080)가 실행 중이어야 한다. 실행 중이 아니면 UX 검토를 건너뛰고 그 사실을 보고한다.
- 전제2: 이번 task scope에서 실제 route 배선을 포함하지 않아 검증 대상 화면이 없으면 해당 단계를 건너뛴다.
