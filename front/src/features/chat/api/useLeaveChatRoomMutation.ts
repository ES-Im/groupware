 ...
 * (LeaveChatRoomDialog)의 onSuccess(navigate('/chat'))를 실행하므로, 아직 대화 상세 화면이
 ...
 *
 * `useCreateChatRoomMutation`과 동일 컨벤션으로 mutation 훅은 캐시 갱신만 담당하고, `/chat`
 * 목록으로의 이동(`navigate`)은 호출부(LeaveChatRoomDialog)의 onSuccess가 수행한다. 실패는
 * 에러를 그대로 던져 호출부가 handleApiError(sonner 토스트)로 처리하도록 둔다.
 */
