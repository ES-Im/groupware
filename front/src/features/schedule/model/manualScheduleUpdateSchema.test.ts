...
/**
 * manualScheduleUpdateSchema(F004, ROADMAP(SCHEDULE) T4.2) 단위 테스트.
 * 전 필드 optional(변경 필드만 전송)이지만, startAt/endAt은 CREATE와 달리 HH:mm:ss 시각만 받는다.
 */
describe('manualScheduleUpdateSchema', () => {
  ...
  it('startAt/endAt이 HH:mm:ss 형식이 아니면 각각 에러다', () => {
    expect(manualScheduleUpdateSchema.safeParse({ startAt: '13:00' }).success).toBe(false)
    expect(manualScheduleUpdateSchema.safeParse({ endAt: '14:00' }).success).toBe(false)
  })

  it('endAt이 startAt 이하면 endAt 경로에 에러가 붙는다', () => {
    const result = manualScheduleUpdateSchema.safeParse({ startAt: '14:00:00', endAt: '13:00:00' })
    expect(result.success).toBe(false)
    if (!result.success) {
  ...
  it('endAt이 startAt과 같으면 endAt 경로에 에러가 붙는다', () => {
    const result = manualScheduleUpdateSchema.safeParse({ startAt: '13:00:00', endAt: '13:00:00' })
    expect(result.success).toBe(false)
  })

  it('startAt/endAt이 함께 유효하게 변경되면 통과한다', () => {
    const result = manualScheduleUpdateSchema.safeParse({ startAt: '13:00:00', endAt: '14:00:00' })
