...
/**
 * 내 출장 신청 이력 조회(`MY_BUSINESS_TRIP_REQUEST_HISTORY`, F733, ROADMAP(DRAFT-BUSINESSTRIP) T4.1 →
 * `GET /api/business-trips/employees/me/request-history`, minRole EMPLOYEE(본인)).
 *
 * **배열 응답(페이징 없음)** — `getDeptBusinessTripHistory`의 `Page<T>`와 달리 최상위가 바로
 * `BusinessTripHistoryEntry[]`다(response-fields.adoc 실측: `[].draftId` 등 배열 인덱스 표기).
 * approvalStatus/yearMonth 쿼리 파라미터는 둘 다 선택값이다(query-parameters.adoc 실측). yearMonth
 * 미입력 시 서버가 현재 월을 기본값으로 적용한다. 값이 없는 파라미터는 쿼리스트링 자체에서 생략되도록
 * params 객체에 조건부로만 채운다(getDeptBusinessTripHistory/getMyAttendanceMonthly와 동일 패턴).
 */
