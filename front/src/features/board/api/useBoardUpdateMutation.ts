 ...
 * invalidate 패턴). 임시저장(초안) 글을 수정하면 제목 등이 바뀌므로 boardKeys.drafts()도 함께
 * invalidate해 임시저장 드롭다운/함이 최신 상태로 반영되게 한다(useBoardRegisterMutation이 등록 시
 * list·drafts를 함께 invalidate하는 것과 동일한 이유). 실패 시 에러는 그대로 던져 호출부(T13.3)가
