...
/**
 * getDeptEmpLeaveSummary(F745, ROADMAP(LEAVE) M4 T4.2) 단위 테스트.
 *
 * apiClient.get을 직접 모킹해 axios 호출 인자(URL에 deptId path param 포함, params)만
 * 검증한다(getDeptLeaveHistory.test.ts와 동일 패턴).
 *
 * keyword/year/page/size는 전부 선택값이므로, 값이 없는 파라미터는 params 객체 자체에서
