...
describe('useCheckInMutation', () => {
  ...
  it('성공(204) 시 attendanceKeys.all이 invalidate되어 하위 쿼리가 재조회되고, 성공 토스트가 노출된다', async () => {
    ...
    const { toast } = await import('sonner')
    expect(toast.success).toHaveBeenCalledWith('출근이 기록되었습니다')
  ...
  it('실패 시 toast.success는 호출되지 않고, handleApiError를 통해 toast.error가 호출된다', async () => {
        ...
        HttpResponse.json(
          {
            ...
            name: 'ATTENDANCE_ALREADY_CHECKED_IN',
            httpStatus: 400,
    ...
    const { Wrapper } = createWrapper()
    const { result } = renderHook(() => useCheckInMutation(), { wrapper: Wrapper })
    ...
    const { toast } = await import('sonner')
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith('이미 출근 처리되었습니다'))
