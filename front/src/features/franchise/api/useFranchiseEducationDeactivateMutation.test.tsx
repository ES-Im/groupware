...
describe('useFranchiseEducationDeactivateMutation', () => {
  it('비활성화는 POST로 요청되고, 성공(204) 시 상세와 캘린더 접두사가 invalidate되어 재조회된다', async () => {
    let isActive1 = true
    let calendarCalls = 0
    ...
    const { result } = renderHook(
      () => ({
        ...
        mutation: useFranchiseEducationDeactivateMutation(),
      }),
      { wrapper: Wrapper },
    )
  ...
  it('서버 판정 실패는 삼켜지지 않고 mutation error로 반영된다', async () => {
    ...
    const { result } = renderHook(() => useFranchiseEducationDeactivateMutation(), {
      wrapper: Wrapper,
    })
