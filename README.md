...
|----------|----------------------------|------------------------------------------|
| 아키텍처     | 계층형 MVC (3 tier)           | 포트-어댑터 (헥사고날)                            |
| 경계       | 패키지 이름                     | 의존 방향 — `domain ← application ← adapter` |
| 도메인 간 연동 | Service가 다른 Service를 직접 호출 | 도메인 이벤트 발행 · 구독                          |
...
>
> 위 팀프로젝트에서 느꼈던 위 문제를 바탕으로 필요한 구조를 고민하였습니다.
> - '코드를 어디에 둘지는' 도메인 역할로 정해져야 할 것이고, 
> - '외부와 만나는 지점'은 비즈니스로직과 별개인 계층에서 모여야한다고 생각했습니다.
> 
> 이번엔, 
